// server_deno.ts
// Deno Deploy + Hono + OpenAI Responses API (GPT-5) — מינימלי ונקי
...
const MODEL = Deno.env.get("OPENAI_MODEL") || "gpt-5"; // תמיד GPT-5 כברירת מחדל
...
// עזר: חילוץ JSON אם ה-LLM עטף בטקסט
...
// פלט ללקוח: { status:"ok", results:[ ... ] } או הודעת שגיאה ידידותית
app.post("/api/search", async (c) => {
  try {
    ...
    // System / User — מבקש JSON קשיח בלבד (ללא schema כדי לשמור על יציבות)
    const system = `
...
`.trim();
    ...
    // ===== OpenAI call (שימו לב: בלי temperature/top_p) =====
